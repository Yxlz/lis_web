...
Ext.ux.MultiCombo = Ext.extend(Ext.form.ComboBox, {
			...
			/**
			 * @cfg {String} highlightClass The css class applied to rows which
			 *      are hovered with mouse selected via key-nav, or highlighted
			 *      when a text-query matches a single item.
			 */
			highlightClass : 'x-grid3-row-over',
			/**
			 * @cfg {Number} autoSelectKey [44] COMMA Sets the key used to
			 *      auto-select an auto-suggest highlighted query. When pressed,
			 *      the highlighted text-item will be selected as if the user
			 *      selected the row with a mouse click.
			 */
			autoSelectKey : 44,
			/**
			 * @cfg {String} allSelectedText Text to display when all items are
			 *      selected
			 */
			allSelectedText : 'All selected',
			/**
			 * @cfg {Number} maxDisplayRows The maximum number of rows to show
			 *      before applying vscroll
			 */
			maxDisplayRows : null,
			...
			initComponent : function() {
				...
				// when blurring out of field, ensure that rawValue contains
				// ONLY items contained in Store.
				this.on('blur', this.validateSelections.createDelegate(this));

				// create an auto-select key handler, like *nix-based console
				// [tab] key behaviour
				...
				this.addEvents(
						 ...
						 *        gives plugins a chance to interact with
						 *        DataView
						 * @author Chris Scott
						 * @param {Combo}
						 *            this
				...
				// when list expands, constrain the height with @cfg
				// maxDisplayRows
				...
				if (forceAll === true || (q.length >= this.minChars)) {
					if (this.lastQuery !== q) {
						...
						if (this.mode == 'local') {
							...
							var last = items.pop();
							this.matches = this.store.data.filterBy(this.store.createFilterFn(this.displayField, new RegExp('^' + last, "i"), false, false)).filterBy(this
									.createTypeAheadFilterFn(items));
				...
				if (this.store.getCount() > 0) {
					...
					if (this.lastQuery == this.allQuery) {
						...
						if (this.matches != null) {
							...
							// @HACK: If store was configured with a proxy, set
							// its mode to local now that its populated with
							// data.
			...
			onTypeAhead : function() {
				if (this.store.getCount() > 0) {
					...
					var rs = this.store.data.filterBy(this.store.createFilterFn(this.displayField, new RegExp(query, "i"), false, false)).filterBy(this
							.createTypeAheadFilterFn(items));
			...
			getCaretPosition : function(raw) {
				...
				if (document.selection) { // <-- IE, ugh:
					// http://parentnode.org/javascript/working-with-the-cursor-position/
					var range = document.selection.createRange();
					// Save the current value. We will need this value later to
					// find out, where the text has been changed
					var orig = obj.value.replace(/rn/g, "n");
					// replace the text
					range.text = text;
					// Now get the new content and save it into a temporary
					// variable
					var actual = tmp = obj.value.replace(/rn/g, "n");
					/*
					 * Find the first occurance, where the original differs from
					 * the actual content. This could be the startposition of
					 * our text selection, but it has not to be. Think of the
					 * selection "ab" and replacing it with "ac". The first
					 * difference would be the "c", while the start position is
					 * the "a"
					 */
					...
					/*
					 * To get the real start position, we iterate through the
					 * string searching for the whole replacement text - "abc",
					 * as long as the first difference is not reached. If you do
					 * not understand that logic - no blame to you, just copy &
					 * paste it ;)
					 */
					for (var index = 0, start = 0; tmp.match(text) && (tmp = tmp.replace(text, "")) && index <= diff; index = start + text.length) {
						start = actual.indexOf(text, index);
					}
				} else if (this.el.dom.selectionStart) { // <-- Go the Gecko
					// way
			...
			// e.g.: if store contains: "betty, barney, bart" and betty is
			// already selected,
			// when user types "b", only "bart" and "barney" should be returned
			// as possible matches,
			...
			/**
			 * setValue Accepts a comma-separated list of ids or an array. if
			 * given a string, will conver to Array.
			 * 
			 * @param {Array,
			 *            String} v
			 */
			setValue : function(v) {
				...
				if (typeof(v) == 'string') { // <-- "1,2,3"
					value = v.match(/\d+/g); // <-- strip multiple spaces and
					// split on ","
			...
			/**
			 * Select an item in the dropdown list by its numeric index in the
			 * list. This function does NOT cause the select event to fire. The
			 * store must be loaded and the list expanded for this function to
			 * work, otherwise use setValue.
			 * 
			 * @param {Number}
			 *            index The zero-based index of the list item to select
			 * @param {Boolean}
			 *            scrollIntoView False to prevent the dropdown list from
			 *            autoscrolling to display the selected item if it is
			...
			/**
			 * Select an item in the dropdown list by its data value. This
			 * function does NOT cause the select event to fire. The store must
			 * be loaded and the list expanded for this function to work,
			 ...
			 *            autoscrolling to display the selected item if it is
			 *            not currently in view (defaults to true)
			 * @return {Boolean} True if the value matched an item in the list,
			 *         else false
			 */
				...
				this.keyNav = new Ext.KeyNav(this.el, {
							...
							"enter" : function(e) {
								...
								if (this.inKeyMode === true) {
									...
									} else if (this.plugins.length && this.highlightIndex == 0 && this.highlightIndexPrev == -1) {
										if (this.plugins[idx]) {
									...
									/**
									 * this block should be moved to method
									 * getCurrentWord
									 * 
									 */
									var pos = this.getCaretPosition(raw);
			...
			// private, blur-handler to ensure that rawValue contains only
			// values from selections, in the same order as selected
			...
			initList : function() {
				if (!this.list) {
					...
					if (!this.tpl) {
						/**
						 * @cfg {String/Ext.XTemplate} tpl The template string,
						 *      or {@link Ext.XTemplate} instance to use to
						 *      display each item in the dropdown list. Use this
						 *      to create custom UI layouts for items in the
						 *      list.
						 *      <p>
						 *      If you wish to preserve the default visual look
						 *      of list items, add the CSS class name
						 * 
						 * <pre>
						 * x - combo - list - item
						 * </pre>
						 ...
						 * {@link #store Store}. An example of a custom
						 * template would be adding an
						 * 
						 * <pre>
						 * ext : qtip
						 * </pre>
						...
						/**
						 * @cfg {String} itemSelector <b>This setting is
						 *      required if a custom XTemplate has been
						 *      specified in {@link #tpl} which assigns a class
						 *      other than
						 * 
						 * <pre>
						 * 'x-combo-list-item'
						 * </pre>
						 * 
						 * to dropdown list items</b>. A simple CSS selector
						 * (e.g. div.some-class or span:first-child) that will
						 * be used to determine what nodes the DataView which
						 * handles the dropdown display will be working with.
						 */
					}

					/**
					 * The {@link Ext.DataView DataView} used to display the
					 * ComboBox's options.
					 * 
					 * @type Ext.DataView
					 */
