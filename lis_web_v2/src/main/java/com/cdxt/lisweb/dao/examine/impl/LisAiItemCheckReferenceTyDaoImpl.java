 ...
 */
@Repository
public class LisAiItemCheckReferenceTyDaoImpl extends BaseDaoImpl<LisAiItemCheckReferenceTy> implements
LisAiItemCheckReferenceTyDao{
	
	@Override
	public Class<LisAiItemCheckReferenceTy> getEntityType() {
		// TODO Auto-generated method stub
		return LisAiItemCheckReferenceTy.class;
	}
	/**
	 * 用于分页查询数据
	...
	public Map<String, Object> findAICRTByPage(int start, int limit,String groupId) {
		...
		//查询智能审核范围分类页面展示的数据
		StringBuffer sql = new StringBuffer("select laicrt.*,lidi.dev_name from LIS_AI_ITEM_CHECK_REF_TY laicrt left join Lis_Inspec_Dev_Info lidi on  laicrt.GROUP_ID=lidi.id  where 1=1");
		//追加根据分组条件
		if (!groupId.equals("null")&&!"".equals(groupId)) {
			sql.append(" and laicrt.GROUP_ID = ? " );
		}
		//查询未删除的类型
		sql.append(" and laicrt.IS_DELETE = '0' " );
		
		sql.append(" order by laicrt.ID desc");
		...
		//查询总条数
		StringBuffer countsql = new StringBuffer("select count(*) from LIS_AI_ITEM_CHECK_REF_TY laicrt left join Lis_Inspec_Dev_Info lidi on  laicrt.GROUP_ID=lidi.id where 1=1");
		//追加根据分组条件
		if (!groupId.equals("null")&&!"".equals(groupId)) {
			countsql.append(" and laicrt.GROUP_ID=  ? " );
		}
		//查询未删除的类型
		countsql.append(" and laicrt.IS_DELETE = '0' " );
	...
	public Map<String, Object> findItemNotAdd(int start, int limit,String groupId, String typeId) {
		...
		if (StringUtils.hasText(typeId)) {
			sqlAdded.append(" and laicr.TYPE_ID = ? " );
		}
	...
	public List<String> findAiItemGroupType(String groupId,String typeName){
		...
		StringBuffer sql = new StringBuffer("select laicrt.id from lis_ai_item_check_ref_ty laicrt where 1=1 ");
		//追加根据分组条件
		if (StringUtils.hasText(groupId)) {
			sql.append(" and laicrt.GROUP_ID = ? " );
		}
		...
		if (StringUtils.hasText(typeName)) {
			sql.append(" and laicrt.TYPE_NAME = ? " );
		}
		//判断是否删除
		sql.append(" and laicrt.IS_DELETE = '0' ");
